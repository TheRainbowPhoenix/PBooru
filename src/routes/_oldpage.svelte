<script>
  ...
  const decryptVariant = async (item, variant, keyBytes, rootUrl, progressCb = null) => {
    ...
    const cacheKey = `pbooru:${variant}:${item.hash}:${variantData.ext}`;
  ...
  const buildTagIndex = () => {
    ...
    items.forEach((item) => {
      const tags = metadataByName.get(item.name)?.tags ?? item.tags ?? [];
      tags.forEach((tag) => {
  ...
  const loadGallery = async () => {
    ...
    try {
      ...
      metadataByName = new Map((metadata.items ?? []).map((item) => [item.name, item]));
      items = manifest.items.map(normalizeManifestItem);
      status = "Done.";
      showPanel("posts");
      const hashId = decodeURIComponent(location.hash.replace(/^#/, ""));
      if (hashId) {
        const selected = items.find((item) => item.hash === hashId);
        if (selected) {
  ...
  const copyLink = async () => {
    if (!modalItem) return;
    const url = `${location.origin}${location.pathname}#${encodeURIComponent(modalItem.hash)}`;
  ...
  const openModal = async (item) => {
    ...
    modalItem = item;
    modalOpen = true;
    location.hash = encodeURIComponent(item.hash);
    modalImageSrc = "";
    fullLoadInFlight = false;
    fullLoaded = false;
    progressValue = 0;
    ...
    const fullCacheKey = `pbooru:full:${item.hash}:${item.full.ext}`;
    ...
    fullAutoTimer = setTimeout(() => {
      if (modalItem?.hash === item.hash && !fullLoaded) {
        loadFull(item);
      }
    }, 4000);
  };
  ...
  const getThumbUrl = async (item) => {
    if (thumbUrls.has(item.hash)) return thumbUrls.get(item.hash);
    const keyBytes = new TextEncoder().encode(key);
    const rootUrl = baseUrl.replace(/\/+$/, "");
    const url = await decryptVariant(item, "thumb", keyBytes, rootUrl);
    thumbUrls.set(item.hash, url);
    return url;
  };

  const getItemTags = (item) => metadataByName.get(item.name)?.tags ?? item.tags ?? [];
  const getItemDate = (item) =>
    metadataByName.get(item.name)?.date ?? item.date ?? "1970-01-01T00:00:00.000Z";
      ...
      <div id="filterBar">
        ...
        {/if}
            ...
            {#each filteredItems() as item}
              <button
                class={`card ${modalItem?.hash === item.hash && modalOpen ? "active" : ""}`}
                type="button"
              ...
              >
                ...
                {:then url}
                  <img src={url} alt={item.name ?? item.hash} loading="lazy" />
                {:catch}
                  <div class="thumb-skeleton"></div>
                {/await}
                <div class="card-info">
